...
**Special Impact:** ⚡ Textbox content displays based on `currentLanguage.code`
...
│  │  Store: currentLanguage = useCurrentLanguage() ⚡                            │  │
...
| Textbox rendering | ⚡ `currentLanguage` | ⚡ `currentLanguage` |
...
// EditorSettingsStore ⚡
currentLanguage = useCurrentLanguage();      // For textbox display
...
DummyCreativeSpace:
  ...
  handleAddDummy(type: DummyType):
    newDummy = addDummy({ type, name: generateDummyName(type, dummies) })
  ...
  RENDER Container (flex row):
    ...
    IF activeDummyId !== null:
      RENDER ManuscriptSpreadView với:
        ...
        // ManuscriptSpreadView uses currentLanguage internally ⚡
    ELSE:
...
- Each dummy has: `id`, `name`, `type` ('prose' | 'poetry'), `spreads[]`
- Dummies are stored in `snapshot.manuscript.dummies[]`

**Language-aware Rendering ⚡**
Components affected by `currentLanguage`:
- **EditableTextbox**: `textbox[currentLanguage.code]?.text`
- **SpreadThumbnail**: Textbox preview uses `currentLanguage.code`
...
### 4.4 Dummy Name Generation

```typescript
function generateDummyName(type: DummyType, existingDummies: ManuscriptDummy[]): string {
